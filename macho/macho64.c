...
void parse_segment_command_64(FILE *p_file, struct load_command_info *lc_info, struct lc_info st_lc_info)
{
    ...
    struct section_64 *p_section_64 = (struct section_64*)malloc(sizeof(struct section_64) * segment_command64.nsects);
    ...
    fread(p_section_64, sizeof(struct section_64), segment_command64.nsects, p_file);

    for (int i = 0; i < segment_command64.nsects; i ++)
    {
        printf("sectname name: %s segname: %s \r\n", p_section_64[i].sectname, p_section_64[i].segname);
        if (p_section_64[i].offset != 0
            && p_section_64[i].flags & S_ATTR_SOME_INSTRUCTIONS
            && ((p_section_64[i].flags & 0xff | S_REGULAR) == 0))
        {
            uint64_t code_len = p_section_64[i].size;

            uint8_t *p_code = (uint8_t*)malloc(sizeof(uint8_t) * code_len);
            if (p_code == NULL)
            {
                continue;
            }

            fseeko(p_file, (off_t)p_section_64[i].offset, SEEK_SET);

            fread(p_code, sizeof(uint8_t), code_len, p_file);
            ...
            while (cnt < code_len)
            {
                size_t count = cs_disasm(handle, p_code + cnt, code_len - cnt, p_section_64[i].offset + cnt, 10, &insn);
                if (count > 0) {
                    // 打印反汇编结果
                    size_t j;
                    for (j = 0; j < count; j++) {
        ...
        else if (p_section_64[i].offset != 0
                 && p_section_64[i].flags & S_CSTRING_LITERALS)
        {
            uint8_t *p_data = (uint8_t*)malloc(sizeof(uint8_t) * p_section_64[i].size);
            if (p_data == NULL)
            {
                continue;
            }
            ...
            fread(p_data, sizeof(uint8_t), p_section_64[i].size, p_file);

            uint64_t size = 0;
            uint8_t *p_str = p_data;
            while (size < p_section_64[i].size)
            {
                if (*((char*)(p_str + size)) <= '\0' || *((unsigned char*)(p_str + size)) >= 0x80)
                {
                    size ++;
                    continue;
                }
                size_t len = strlen((const char*)p_str + size);
                printf("%s \r\n", p_str + size);
                size += (len + 1);
            }
